...
function call(chcy::chcy, eps)
    m_spl = Spline1D(chcy.x, chcy.m)
    dm = Dierckx.derivative(m_spl, eps, nu=3)
    return -((eps-chcy.x_req)*log(abs(eps - chcy.x_req)) - (eps - chcy.x_req))*dm
end
...
while t < final_t
    ...
    #Update edge velocity through interaction
    for i = 1:Nx+2
        m[i] = ue.value[i]*del.value[i]
    end

    for i = 2:Nx+1
        ch = chcy(x,m[2:Nx+1],x[i-1])
        ueadd[i] = (1./(pi*sqrt(re)))*quadgk(ch, 0., x[i-1], pi)[1]
        ue.value[i] = ue.value[i] + ueadd[i]
    end
    uex_spl = Spline1D(x,ue.value[2:Nx+1])
    for i = 2:Nx+1
        uex.value[i] = Dierckx.derivative(uex_spl, x[i-1])
        uet.value[i] = ueadd[i]/dt
    end
